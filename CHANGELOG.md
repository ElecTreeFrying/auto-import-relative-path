...
- **Drag-and-drop import from Explorer.** Drag any supported source file from the Explorer sidebar into an open editor — the extension generates the same import snippet as the paste commands and inserts it on its own line at the drop point, matching the surrounding indentation (a drop onto a blank line reuses that line), never spliced into the middle of the line it lands on. Registered as a `DocumentDropEditProvider` for all 13 supported destination languages. No keybinding needed; no new settings — uses the same styles and configuration as the paste commands. Unsupported pairs show the same "Cannot import" warning as paste commands and insert nothing — the provider suppresses VS Code's default raw-path drop.
...
- **Gating logic extracted.** The twelve-clause extension-pair check is now a shared `isPairSupported()` function in `src/gating.ts`, reused by both commands and the drop provider.
