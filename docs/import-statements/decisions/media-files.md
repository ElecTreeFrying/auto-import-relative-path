...
Both settings are well under the ~7-entry ceiling — no picker-bloat risk. The "Picker inventory (counts)" table in [`../spec/statements.md`](../spec/statements.md) carries the HTML media rows alongside CSS.
...
- **Markdown native media** — *(Doc-local rejection — no clean criterion fit; closest is the syntax-ceiling principle.)* Markdown has no native `<video>` / `<audio>` syntax; there is no canonical shape to emit. Users embed raw HTML when needed. (Closest is the syntax-ceiling principle — CRITERIA.md applies it to single-shape picker rows; the Markdown image inventory row in `../spec/statements.md` sits at 3 entries, all expressible in Markdown or its HTML `<img>` escape hatch, with no canonical native-media shape left to add.) Skip.
