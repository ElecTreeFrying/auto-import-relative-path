...
**Yes.** Open VS Code's keyboard shortcuts editor (<kbd>Ctrl</kbd>/<kbd>Cmd</kbd>+<kbd>K</kbd> <kbd>Ctrl</kbd>/<kbd>Cmd</kbd>+<kbd>S</kbd>), search for `auto-import.copyFilePath`, `auto-import.pasteImport`, or `auto-import.copyPaste`, and rebind. Useful if `Cmd+I` clashes with another extension you use.

The five palette-only commands — `auto-import.pasteImportWithStyle`, `auto-import.setDefaultImportStyle`, `auto-import.setImportPlacement`, `auto-import.togglePreserveScriptExtension`, and `auto-import.resetImportStyles` — have no default keybinding; assign one from the same editor if you use them often.

**Already bound the older `extension.*` ids?** They still work. Every command keeps its original `extension.<name>` id registered alongside the `auto-import.<name>` one, so a keybinding, macro, or `tasks.json` entry made before the rename fires exactly as it did. The older ids are hidden from the Command Palette so each command appears once, but they remain fully bindable — search the keyboard shortcuts editor with **Show User Keybindings** if you want to see the ones you already have.
...
**Fix:** Rebind from VS Code's keyboard shortcuts editor (<kbd>Ctrl</kbd>/<kbd>Cmd</kbd>+<kbd>K</kbd> <kbd>Ctrl</kbd>/<kbd>Cmd</kbd>+<kbd>S</kbd>). The command IDs to search for are `auto-import.copyFilePath`, `auto-import.pasteImport`, and `auto-import.copyPaste` — the only three with default keybindings. The five palette-only commands (`auto-import.pasteImportWithStyle`, `auto-import.setDefaultImportStyle`, `auto-import.setImportPlacement`, `auto-import.togglePreserveScriptExtension`, `auto-import.resetImportStyles`) accept assignments from the same editor if you want to give them a key.
