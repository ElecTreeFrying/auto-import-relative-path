...
| Insertion position | Configurable (Top / Bottom / Cursor) | Computed by `computeImportPlacement()` (Top / Bottom / Cursor) — drop line used as Cursor input; the import lands on its own line at the target line's structural indent (a blank target line is reused in place), while paste keeps the caret column for HTML / Markdown / LaTeX |
...
When **every** member is skipped, nothing is inserted and a single warning reports the most informative failure, in this precedence: unsupported pair > source not found > same file > no extension > empty clipboard. (The drop provider's set is the applicable subset: unsupported pair > same file > no extension.)
...
The copy/paste commands — Copy File Path, Paste as Import, Paste as Import (Pick Style), Set Default Import Style, and the composite Insert Import from Selected File — clear existing notifications before executing, so a toast from a previous command is dismissed when a new one starts. The three settings commands (Set Import Placement, Toggle Preserve Script File Extension, Reset All Import Styles) emit their info toast without clearing.
