...
| Insertion position | Configurable (Top / Bottom / Cursor) | Computed by `computeImportPlacement()` (Top / Bottom / Cursor) — drop line used as Cursor input; the import lands on its own line (column 0), while paste keeps the caret column for HTML / Markdown / LaTeX |
