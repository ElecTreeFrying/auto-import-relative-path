...
VS Code extension that generates relative-path import statements for JS/TS/JSX/TSX/MDX/CSS/SCSS/HTML/Markdown/Vue/Svelte/Astro/LaTeX. Input gestures: **copy-paste** (the copy/paste command set) and **drag-and-drop** (a `DocumentDropEditProvider` covering every destination language). Both gestures accept Explorer multi-selections — the clipboard channel and the drop provider fan out per file and insert one stacked block, tab stops renumbered via `snippets/compose.ts`; the style-picker commands instead reduce a multi-selection to its primary member. The commands (`auto-import.copyFilePath`, `auto-import.pasteImport`, `auto-import.copyPaste`, `auto-import.pasteImportWithStyle`, `auto-import.setDefaultImportStyle`, `auto-import.setImportPlacement`, `auto-import.togglePreserveScriptExtension`, `auto-import.resetImportStyles`) are registered in `src/extension.ts` from the `COMMAND_HANDLERS` table, which registers every entry under **both** prefixes in `COMMAND_PREFIXES` — the canonical `auto-import.*` id and the original `extension.*` id it shipped under. The legacy family stays registered permanently so existing user keybindings keep firing (VS Code has no command-alias mechanism), and `contributes.menus.commandPalette` hides it with `when: "false"` so nothing appears twice in the palette. The keybound commands (`copyFilePath` — `cmd/ctrl+shift+a`, `pasteImport` — `cmd/ctrl+i`, `copyPaste` — `alt+d` in the explorer) get their bindings from `package.json`; the rest are reachable via the Command Palette (and `pasteImportWithStyle` also via the `copy-success` toast button). The drop provider is registered alongside the commands in `activate()` and uses the same snippet pipeline.
...
npm run qa               # full automated QA sweep: qa:core (headless suite) + qa:ui (ExTester UI suite)
npm run qa:ui            # UI layer only — ExTester-driven VS Code (pickers, toasts, settings UI, native drag)
```
...
- **Two-site button-label sync** — toast action-button labels in `editor/notification.ts` must match the `switch` cases in the dispatching module character-for-character, at these pairs: the `copy-success` buttons ↔ `commands/copy-file-path.ts`, the `styles-reset` **Undo** ↔ `commands/reset-import-styles.ts`, and the `review-request` buttons ↔ `editor/review-prompt.ts`. Labels stay free of apostrophes — the byte-check tests extract single-quoted literals. See [`src/commands/CLAUDE.md`](src/commands/CLAUDE.md).
...
- **Manifest-string localization sync — package.nls** — every display string in `package.json` (`displayName`, `description`, command titles, `configuration.title`, each setting's title / description / enumDescriptions) is a `%key%` placeholder resolved from `package.nls.json` (the English base and per-key fallback) and its locale siblings (`zh-cn`, `es`, `fr`, `pt-br`, `ru`, `de`, `ja`, `tr`). Rewording or adding such a string means editing the base **and every locale file** — a key missing from a locale silently falls back to English there. The legacy `extension.*` command entries resolve through the same keys as their `auto-import.*` twins — never mint separate legacy keys. Localized titles keep the literal `Auto Import` brand prefix; only the action text is translated (`fr` separates with ` : `). Configuration `enum` values and `default`s are **never** localized — they are the same runtime-matched strings the three-site config sync keeps byte-identical; only their descriptions are display text. Runtime strings (toasts, pickers, button labels) are likewise not localized — the two-site button-label byte-sync depends on those literals. The nls files ship in the VSIX; `.vscodeignore` must never match them.
...
- `process/` is gitignored — private publishing notes + access tokens; never commit it. `import-statement-design/` (the design library) and `qa/` (the manual-QA corpus, including the `qa/staging/` roadmap-item sessions) are gitignored too — kept locally for development, never tracked or shipped. `.claude/` is gitignored in full — the old `!agents/ !skills/ !workflows/` carve-out was removed on 2026-07-25 (nothing under it had ever been committed), so this repo now matches the other three.
