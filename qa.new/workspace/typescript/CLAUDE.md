# qa.new/workspace/typescript/CLAUDE.md
...
| `rejected/` | Non-`.ts` files (21 extensions) for gating rejection tests. Most are empty placeholders — content doesn't matter, only the file extension. |
...
- **`rejected/*`** — content is irrelevant. Only the file extension matters for gating tests. Binary-type files (`.png`, `.mp4`, `.woff2`, `.mp3`, `.pdf`) are empty placeholders.
