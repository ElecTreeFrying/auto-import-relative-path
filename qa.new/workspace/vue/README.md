# qa.new/workspace/vue/

Fixtures for the Vue destination checklist ([`checklists/vue.md`](../../checklists/vue.md)).

`.vue` is the pipeline's **first framework-trio destination** — `.vue`/`.svelte`/`.astro` all share one
...
`.vue` is **allow-list** (`VUE_SUPPORTED_EXTENSIONS`), so non-member sources are gate-rejected. Unlike the
other allow-list workspaces (`javascript/`, `css/`, …) there is **no `rejected/` dir** — `vue.md`
co-locates the 9 gated-out fixtures inside **`assets/`** alongside the 8 accepted non-script sources.
Every fixture below is referenced by `vue.md`, so the directory is checklist↔workspace 1:1 with no orphans.
...
|---------|-----------|---------|
| `src/App.vue` | 1–10 | Primary paste/drop **destination** (the open editor; empty `<script setup>`) |
...
| `src/angular/widget.component.js` | 5.8 | Angular suffix on a `.js` source → **still** PascalCase (`.vue` ≠ `.tsx`) |
| `src/angular/2fa.service.ts` | 5.10 | Angular suffix, illegal derived id (leading digit) → bare `$1` |
| `src/classes/event-bus.ts` | 5.7 | `export class EventBus` → still bare `$1` (`.vue` never reads classes) |
...
| `assets/BaseButton.vue` | 1.5, 10.3 | `.vue` self-source → **asset-routed** `${1:name}` (the `.vue`→`.vue` quirk) |
...
| `destinations/setup-and-instance.vue` | 6.1 | `<script setup>` wins over instance `<script>` (block-selection preference) |
| `destinations/with-imports.vue` | 6.2.1, 6.3.1, 6.4.1, 6.4.2, 9.4, 9.5 | Two imports — Bottom/Top/Cursor placement tests |
...
|-----------|-------|---------|
| `src/` | 14 | Script sources (all four exts → TS arm) + nested source + `angular/` (7) + `classes/` (1) + primary destination `App.vue` |
| `assets/` | 17 | Non-script sources — **8 accepted** (fixed shapes) + **9 rejected** (gated out, co-located; no `rejected/` dir) |
| `destinations/` | 8 | Pre-filled SFC `<script>`-block placement-test destinations |
| **Total** | **39** |
