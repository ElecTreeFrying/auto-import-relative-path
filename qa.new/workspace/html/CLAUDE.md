# qa.new/workspace/html/CLAUDE.md
...
| `src/images/` | `logo.png` — the image source (→ `<img>`, 3 styles). Empty placeholder — `.html` keys on the `.png` extension, not bytes. |
| `src/media/` | `intro.mp4` (video → `<video>`, 4 styles), `theme.mp3` (audio → `<audio>`, 2 styles), `captions.vtt` (text-track → fixed `<track>` with `${1:en}`/`${2:English}` tab stops). `.mp4`/`.mp3` are empty placeholders; `captions.vtt` holds a minimal `WEBVTT` cue. |
...
| `rejected/` | One source per rejected extension (8 files) for allow-list gating. Content is irrelevant — only the extension matters. `page.html` drives the dedicated `.html → .html` rejection. |
...
- **`rejected/*`** — content is irrelevant; only the extension matters for gating. `font.woff2` and `doc.pdf` are empty 0-byte placeholders; the six text stubs hold a one-line body.
