...
2. Read destination from `document.uri.fsPath` (passed by the API — no active-editor lookup) and `document.getText()` once.
3. **Fan out** over every source path, collecting `DropCandidate`s (`{ value, placement }`). Per source, in order: **same-file check** (case-insensitive) → skipped, counted; **gating** via `isPairSupported(info)` (shared with `commands/`) → skipped, the offending pair remembered for the toast; async `buildImportSnippet(info)`, empty/newline-only value → skipped; otherwise **placement** via `computeImportPlacement(...)` (same Top/Bottom/Cursor/Astro/SFC logic as the command flow, parameterised with the drop position) is computed and the candidate kept.
4. **No surviving candidate** → mirror the single-file toasts (`'not-supported'` if any pair was unsupported, else `'same-file-path'`) and return `suppressDrop()`.
