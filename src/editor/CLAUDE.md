...
- Producers: `commands/paste-import.ts` raises (`'same-file-path'`, `'not-supported'`, `'no-active-editor'`, `'no-extension'`, `'empty-clipboard'`, `'source-not-found'`); `commands/paste-import-with-style.ts` raises the same set plus its branch on `variants.length`; `commands/copy-file-path.ts` raises (`'no-file-to-copy'`, `'no-extension'`, `'copy-success'`); `commands/set-default-import-style.ts` raises the same rejection set plus `'no-configurable-style'` and `'default-style-saved'`; `drop/provider.ts` raises (`'same-file-path'`, `'not-supported'`); the settings commands raise their own info toasts — `'placement-saved'` from `commands/set-import-placement.ts`, `'preserve-script-extension-toggled'` from `commands/toggle-preserve-script-extension.ts`, and `'no-styles-to-reset'` / `'styles-reset'` / `'styles-restored'` from `commands/reset-import-styles.ts`.
