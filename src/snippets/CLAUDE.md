...
- **Component-like sources** `.md`/`.mdx`/`.vue`/`.svelte`/`.astro` → `import ${1:name} from '<path>';` — kept generic; their conventional identifier is PascalCase, a deferred default-import pathway (see `docs/import-statements/future/framework-roadmap.md`), so they are *not* basename-derived.
...
The picker flow's `variants.ts:buildReactNonScriptVariant` and `languages/framework-component.ts` both call this same `buildAssetImportStatement`. One switch, shared by all its callers — no parallel copy to keep in sync.
...
The default-import positions across every builder pre-fill their `${1:…}` binding with an identifier derived from the source basename, so a paste/drop arrives with a sensible name selected instead of an empty `$1`. The single derivation lives in `path/import-name.ts:deriveImportName` (splits the extension-stripped basename on `-`/`_`/`.`/space, camelCases, **preserves the first segment's case** so `App.jsx` → `App` and `logo.svg` → `logo`, validates against `/^[A-Za-z_$][\w$]*$/`, returns `null` on failure). Its consumers:
...
- **Asset switch** (`_react.ts:buildAssetImportStatement`) — the plain-asset name group and the media/text-track url group (component-like sources keep the generic `${1:name}`; CSS-modules keeps `${1:styles}`).
