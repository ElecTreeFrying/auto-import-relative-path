...
| `scss.ts` | `buildSnippet`, `buildScssImportSnippetByStyle`, `prepareScssImportPath` | SCSS `@use`/`@forward`/`@import` (5 styles); strips partial underscores, always keeps `.css`. Image sources defer to `css.ts`. |
...
| `framework-component.ts` | `buildSnippet` | Vue/Svelte/Astro entry — defers to `buildTypeScriptImportSnippet` for every source; strips the script extension per the preserve setting. All three share identical semantics. |
