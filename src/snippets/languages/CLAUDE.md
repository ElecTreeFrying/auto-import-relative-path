...
- **Styled languages** (`javascript.ts`, `typescript.ts`, `css.ts`, `scss.ts`, `html.ts`, `markdown.ts`) — each exposes a pure `buildXImportSnippetByStyle(styleIndex, relativePath)` switch (except `typescript.ts`, whose switch adds an optional third parameter `detectedImportName` for class-name pre-fill at style 0). The config lookup sits in a `buildXImportSnippet` wrapper for JavaScript, TypeScript, and CSS; SCSS and HTML resolve config inline in `buildSnippet` instead. (`scss.ts` additionally exports `prepareScssImportPath`, a path-normalizer called by `variants.ts`.) See the config/pure split below.
