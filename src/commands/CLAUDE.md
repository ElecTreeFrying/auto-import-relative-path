...
- `toggle-preserve-script-extension.ts` — flips the `auto-import.importStatement.script.preserveScriptFileExtension` boolean (`script` / `preserve`, `?? false` when unset) and emits `'preserve-script-extension-toggled'` (`On` / `Off`). No QuickPick. **Living-gate note:** the planned v1.1.0 tri-state enum (design archived at `.claude/_archive/future/auto-detect-extensions.md`) must reconcile this 2-state toggle through a 3-way picker or removal.
