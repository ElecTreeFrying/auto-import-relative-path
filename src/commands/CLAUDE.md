...
- `toggle-preserve-script-extension.ts` — flips the `auto-import.importStatement.script.preserveScriptFileExtension` boolean (`script` / `preserve`, `?? false` when unset) and emits `'preserve-script-extension-toggled'` (`On` / `Off`). No QuickPick. **Living-gate note:** the v1.1.0 tri-state enum planned in the local design library (`docs/import-statements/future/auto-detect-extensions.md`) must reconcile this 2-state toggle through a 3-way picker or removal.
