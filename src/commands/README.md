...
| `reset-import-styles.ts` | `executeResetImportStyles` | Settings command — no gating. Clears every customized import-style Global override (the nine configurable styles) back to its `package.json` default; shows an info toast if none are customized, otherwise a toast with an **Undo** action. |
