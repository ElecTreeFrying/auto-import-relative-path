...
// language ID; `.mdx` is matched by file PATTERN — VS Code ships no `mdx` language and we contribute none,
// so a `.mdx` file may open as plaintext, which a language-ID selector would miss (the drop would then
// fall back to VS Code's raw-path insert). The glob matches the file regardless of language, mirroring how
// the paste commands key off `path.extname`. Pin both so a new destination can't ship drag-and-drop
// without a matching selector entry.
...
describe('drop/selector', () => {
  ...
  it('registers all 12 destinations — 11 by language ID, `.mdx` by file pattern', () => {
    ...
    assert.deepStrictEqual(patterns, [ '**/*.mdx' ], '`.mdx` must be matched by file pattern, not language ID');
  });
