...
| `commands/` | Command-level tests (`copy-file-path`, `copy-paste`, `paste-import`, `paste-import-with-style`, `set-default-import-style`, `reset-import-styles`) — most of the commands. The settings-only `set-import-placement` and `toggle-preserve-script-extension` have no command-level test; they're covered by the activation/registration smoke checks in `extension.test.ts` |
...
| `snippets/languages/` | Per-language snippet builder tests (one per destination-language builder — incl. `latex` — plus `framework-component` for the shared Vue/Svelte/Astro path) |
| `fixtures/` | Self-contained fixture tree (paste sources + destinations) opened by the fixture-driven tests via `FIXTURE_ROOT`; excluded from `tsc`/ESLint |
