...
- **Explain WHY for non-obvious decisions.** Examples in this repo worth their tokens: legacy-Angular index-1 PascalCase substitution (`snippets/typescript.ts:generateAngularLegacyImportName`); the `./` prefix rule (`path/relative.ts:computeRelative`); SCSS partial-filename stripping (`snippets/scss.ts:normalizePartialFilename`); the `.css`-always-preserved-in-SCSS asymmetry (`snippets/scss.ts:determineScssExtension`).
...
**Private helpers and union-literal variants are opt-in, not blanket.** A private helper earns TSDoc only when it encodes a non-obvious rule (`snippets/typescript.ts:generateAngularLegacyImportName`'s legacy-Angular substitution, `snippets/scss.ts:normalizePartialFilename`'s partial-stripping). A union variant earns TSDoc only when it carries a hidden contract the type name and surrounding context don't reveal. A helper named `toUnixPath` whose body is `filePath.replace(/\\/g, '/')` does not earn one.
...
| TSDoc on private helpers | opt-in | Only when the helper encodes a non-obvious rule (legacy-Angular substitution, SCSS partial-stripping) |
...
### Keep — `snippets/typescript.ts:generateAngularLegacyImportName` TSDoc (already in repo)

The legacy Angular `.component` / `.directive` / `.pipe` / `.service` / `.module` filename convention drives a PascalCase substitution that nothing else in the codebase signals. The TSDoc is the only mechanism that explains why `app-root.component.ts` becomes `{ AppRootComponent }`. §2(3) load-bearing — keep as-is when Pass-B runs.
